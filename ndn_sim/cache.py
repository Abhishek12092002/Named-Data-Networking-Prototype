...
class CoordinatedCache(BaseCache[K, V]):
    """LRU cache whose *admission* is gated by an SDN controller's directive.

    Instead of every node greedily caching whatever content passes through
    it, a controller (see ``controller.SDNController.coordinate_cache_placement``)
    decides which names each node is allowed to cache, based on a global view
    of popularity. Eviction/lookup behavior is unchanged (still LRU); only
    admission is restricted to controller-approved names.

    If no directive has been set (``allowed`` is empty), this behaves exactly
    like a plain LRU cache, so the SDN caching feature is opt-in.
    """

    def __init__(self, capacity: int, **kwargs):
        super().__init__(capacity, **kwargs)
        self._inner = LRUCache(capacity, **kwargs)
        self.stats = self._inner.stats
        self.allowed: set = set()

    def set_allowed(self, allowed: set) -> None:
        self.allowed = allowed

    def get(self, key: K, now_ms: float = 0.0) -> Optional[V]:
        return self._inner.get(key, now_ms=now_ms)

    def put(self, key: K, value: V, now_ms: float = 0.0) -> None:
        if self.allowed and key not in self.allowed:
            return
        self._inner.put(key, value, now_ms=now_ms)
...
def make_cache(policy: str, capacity: int, admit_threshold: int = 1, ttl_ms: float | None = None, seed: int = 0) -> BaseCache[str, bytes]:
    ...
    if p == 'sdn-coordinated':
        return CoordinatedCache(capacity, **common)
